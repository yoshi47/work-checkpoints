...
#   delete-checkpoints.sh --codex                    Codex作成分をすべて削除（リネーム済みは保護）
...
if [ "$MODE" = "--all" ]; then
  ...
  echo "Deleted $COUNT Codex-created checkpoint(s)."
  ...
  echo "  delete-checkpoints.sh --codex                  Delete all Codex-created checkpoints"
